...
const Navbar = () => {
    ...
    const { user, setUser, setShowUserLogin, navigate , setSearchQuery , searchQuery, getCartItemsCount } = UseAppContext()
        ...
        <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white relative transition-all">
                    ...
                    (
                        <div className="relative group">
                            ...
                            <ul className='hidden group-hover:block absolute top-10 right-0 bg-white shadow border border-gray-200 py-2.5 w-30 rounded-md text-sm z-40'>
                                <li onClick={() => navigate('/myorders')} className='p-1.5 p1-3 hover:bg-primary/10 cursor-pointer'>My Orders</li>
            ...
            {open && (
                <div className={`${open ? 'flex' : 'hidden'} absolute top-[60px] left-0 w-full bg-white shadow-md py-4 flex-col items-start gap-2 px-5 text-sm md:hidden`}>
                    ...
                    {user &&
                        <NavLink to='/myorders' onClick={() => setOpen(false)}>My Order</NavLink>
                    }

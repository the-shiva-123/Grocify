...
const ProductDetails = () => {
    const { Products: products, navigate, currency, addToCart } = UseAppContext()
    const { id } = useParams()
    ...
    return product && (
        <div className="mt-12">
            ...
            <div className="flex flex-col md:flex-row gap-8 mt-4">
                {/* Left side: Thumbnails in vertical column */}
                <div className="flex flex-row md:flex-col gap-3 overflow-x-auto md:overflow-x-visible">
                    {product.image.map((image, index) => (
                        <div
                            key={index}
                            onClick={() => setThumbnail(image)}
                            className={`border w-16 h-16 md:w-20 md:h-20 flex-shrink-0 ${thumbnail === image ? 'border-primary border-2' : 'border-gray-500/30'} rounded overflow-hidden cursor-pointer hover:border-primary transition`}
                        >
                ...
                {/* Main image */}
                <div className="border border-gray-500/30 rounded overflow-hidden flex-1 max-w-md">
            ...
            <div className="flex flex-col items-center mt-20 ">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4  sm:grid-cols-3 gap-4 md:gap-6 lg:grid-cols-5 mt-6 w-full">
